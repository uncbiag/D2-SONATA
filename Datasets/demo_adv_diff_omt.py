...
#%% Basic settings
parser = argparse.ArgumentParser('2D demo: anisotropic flying points [V as time-varying]')
...
parser.add_argument('--lesion_mask', type = bool, default = False, help = 'determines whether adding random-sized value mask for (stochastic) uncertainty supervision')
...
# NOTE: total time pointsc := (sub_collocation_nt - 1) * n_collocations
...
parser.add_argument('--GT_D_type', type = str, default = 'scalar', choices = ['constant', 'scalar', 'diag', 'full_cholesky', 'full_soectral'])
...
class AniPointsGenerator(object):
    ...
    def __init__(self, args, device = 'cpu', save_fld = None, to_save = True, for_test = False):
        ...
        self.t0, self.dt, self.n_collocations, self.sub_collocation_nt = \
            args.t0, args.dt, args.n_collocations, args.sub_collocation_nt
    ...
    @property
    def T_collocation(self):
        '''
        all T
        '''
        t = self.t0 + np.arange(self.n_collocations) * (self.dt * (self.sub_collocation_nt - 1))
    ...
    def Vlst(self):
        ...
        if 'diff_only' in self.perf_pattern:
            ...
            if self.args.save_as_addition:
                ...
                Vlst.update({'orig_V': V.unsqueeze(0), 'delta_V': delta_V.unsqueeze(0)})
    ...
    def integrate(self, PDEFunc, to_print = True):
        ...
        # save v_lst
        self.Vx_series, self.Vy_series = PDEFunc.get_V_series(self.T_collocation) # (nT-1, r, c)
        if self.save_fld:
            self.save(self.Vx_series, 'Vx_series')
            self.save(self.Vy_series, 'Vy_series')
        ...
        with torch.no_grad():
            for i_coll in range(1, self.n_collocations):
                ...
                # update current V(t) 
                PDEFunc.Vlst = {"Vx": self.Vx_series[i_coll - 1].unsqueeze(0), "Vy": self.Vy_series[i_coll - 1].unsqueeze(0)}
                for i_sub_coll in range(1, self.sub_collocation_nt): # (nt = -1, batch = 1, slc, row, col)
    ...
    def get_movie(self, PDEFunc, ToNormalize = True, to_print = True):
        # initialization: V0, D0, V_series, D_series
...
def randn_generate(save_fld = None, to_save = False, to_print = True, for_test = False, device = 'cpu'):   
    ...
    param_lst = {'Dlst': Generator.Dlst, 'Llst': Generator.Llst, 'Ulst': Generator.Ulst, 'Slst': Generator.Slst, 'Vlst': Generator.Vlst, \
        'Sigma_V': Generator.UncertaintyMask_V, 'Sigma_D': Generator.UncertaintyMask_D, 'Sigma': Generator.UncertaintyMask, 'Seg_V': Generator.LesionSegMask_V, 'Seg_D': Generator.LesionSegMask_D, \
            'ValueMask_D': Generator.ValueMask_D, 'ValueMask_V': Generator.ValueMask_V, 'Vx_series': Generator.Vx_series, 'Vy_series': Generator.Vy_series}
...
if __name__ == '__main__':
    ...
    postfix = '_omt' if args_demo.V_time else ''
    save_dir = os.path.join(main_dir, args_demo.GT_perf_pattern + postfix)
