...
def test_odf_with_zeros():
    ...
    data = image.load_nifti_data(fdata)
...
def test_tensor_model():
    fdata, fbval, fbvec = get_fnames('small_25')
    data1 = image.load_nifti_data(fdata)
    gtab1 = grad.gradient_table(fbval, fbvec)
    data2, gtab2 = dsi_voxels()
    print(data1.shape)
    print(gtab1.gradients.shape)
    print(data2.shape)
    print(gtab2.gradients.shape)
    for data, gtab in zip([data1, data2], [gtab1, gtab2]):
        ...
        dtifit = dm.fit(data)
        print(dtifit.fa[0, 0, 0])
        print(dtifit.model_params[0, 0, 0][:3])
        print(dtifit.model_params[0, 0, 0][3:].reshape(3, 3))
